...
name: - 'Renita Kurian, Dept. of CSE - PES1UG20CS331'
  output:
  pdf_document:
    fig_width: 6
    fig_height: 6
...
```{r}
model_full <- lm(energy~(danceability+key+loudness+mode+speechiness+acousticness+instrumentalness+liveness+valence+tempo+duration_ms+time_signature), data = df1)
summary(model_full)

```
The standard deviation of residuals is 0.4077. 
The variance is energy is explained by the variables by 84.4%.
Since p value is less than 0.05, there is a significant relation between energy and the other features. 
...
The following features - loudness, acousticness, valence - have the highest correlations with energy
...
```{r}
model <- lm(energy~(danceability+loudness+acousticness+valence), data = df1)
summary(model)
```
The partial f-tests below show that key, mode, speechiness, instrumentalness, liveness, tempo, duration, time_signature are not important features for the prediction of energy
...
Null Hypothesis - All coefficients of attributes removed from model is zero
Alternate Hypothesis - Atleast one of the removed coefficients is zero

Partial f-test for mode, key, time signature, valence
```{r}
m1 <- lm(energy~(danceability+loudness+speechiness+acousticness+instrumentalness +tempo+duration_ms), data = df1)
anova(m1, model_full)
```
P value is less than 0.05 and hence, null hypothesis is rejected.

Partial f-test for tempo, liveness, key, time signature
```{r}
m2 <- lm(energy~(danceability + loudness + speechiness + acousticness + instrumentalness + valence + duration_ms), data = df1)
anova(m2, model_full)
```
P value>0.05 and hence, null hypothesis is not rejected.


```{r}
m3 <- lm(energy~(danceability+loudness+acousticness+valence+instrumentalness+mode), data = df1)
anova(m3, model_full)
```

Partial f-test for key, mode, speechiness, instrumentalness, liveness, tempo, duration, time_signature
```{r}
model <- lm(energy~(danceability+loudness+acousticness+valence), data = df1)
anova(model, model_full)
```
Since p value is greater than 0.05, null hypothesis cannot be rejected and hence the removed variables do not significantly improve the model. 
...
```{r}
library(olsrr)
AICmodel <- lm(energy ~ (danceability+key+loudness+mode+speechiness+acousticness+instrumentalness+liveness+valence+tempo+duration_ms+time_signature), data=df)
ols_step_backward_aic(AICmodel)
```

```{r}
ols_step_both_p(AICmodel)
```
The variables loudness, acousticness, danceability, valence, mode and instrumentalness are important features to predict energy
...
ols_vif_tol(model)
```
Multicollinearity is not highly likely since none of the VIF values are very high.

```{r}
ols_plot_cooksd_bar(model)
```
```{r}
df_new <- spotify_df[-c(12,30,49,59,159,162,191,184,171,172),]
new_model <- lm(energy~ danceability+loudness+acousticness+valence, data=df_new)
summary(new_model)         
```
The fit improves on removing the outliers.
